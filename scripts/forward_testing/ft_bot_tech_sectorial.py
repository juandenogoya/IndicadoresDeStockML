...
from src.trading.strategy_technical import calcular_score_tecnico
...
def obtener_indicadores_con_sector() -> list[dict]:
    ...
    JOIN con activos para traer el sector de cada ticker.
    Solo incluye sectores en SECTORES_ACTIVOS.
    """
    engine = get_engine()
    with engine.connect() as conn:
        rows = conn.execute(text("""
            SELECT i.ticker, a.sector,
                   i.dist_sma21, i.dist_sma50, i.dist_sma200,
                   i.rsi14, i.macd, i.macd_signal, i.macd_hist,
                   i.adx, i.vol_relativo, i.atr14
            FROM (
