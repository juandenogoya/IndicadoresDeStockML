"""
ft_bot_smc_v3.py
Forward-testing -- SMC CHoCH/BOS sobre estructura CONFIRMADA (sin datos futuros).

Instancias: FT_SMC_v3_N5 y FT_SMC_v3_N3 (un proceso por ventana, --ventana).

QUE CAMBIA CONTRA FT_SMC_v1 (docs/estructura_velas.md sec. 9.3)
    La REGLA es la misma, letra por letra: mismo score, mismos filtros
    obligatorios, mismo trailing SL estructural, mismas prioridades de salida,
    mismo tamano de posicion. Lo unico que cambia es la FUENTE de la estructura:

        v1  -> features_market_structure (swings con ventana centrada; la ultima
               fila trae swings PROVISIONALES que pueden desaparecer manana)
        v3  -> features_estructura (swings CONFIRMADOS: el swing de la barra p
               existe en p+N, cuando la ventana derecha esta completa) y
               features_velas (patrones clasicos con contexto)

    Y la ventana N: v1 usa N=10; aca N=5 y N=3, las dos variantes que en el
    backtest 2021-09 -> 2026-09 pasaron la regla pre-registrada (+59,8% y +58,2%
    contra +13,5% de N=10 confirmado). El backtest no alcanza para elegir entre
    N=5 y N=3: coinciden en el total y difieren por anio, asi que las dos corren
    en paralelo y decide la operacion real. FT_SMC_v1 queda como control.

    Diferencia menor, del lado correcto: el lookback de eventos se ancla a la
    ultima rueda de DATOS y no al reloj (ver ft_scoring_estructura).

    Ninguna de las dos usa la historia con futuro: en vivo el bot lee la ultima
    fila disponible. El problema de la v1 no es que vea el futuro hoy, es que su
    senal es inestable y que su backtest no era reproducible en vivo.
...
Uso:
    python scripts/forward_testing/ft_bot_smc_v3.py --ventana 5
    python scripts/forward_testing/ft_bot_smc_v3.py --ventana 3 --dry-run
"""
...
# El SCORE se importa del modulo de la v1: la logica de decision no se duplica.
from scripts.forward_testing.ft_scoring import (
    calcular_score_estructura,
    _swing_low_precio,
)
# Los DATOS salen de la historia sin informacion futura.
from scripts.forward_testing.ft_scoring_estructura import (
...
# -- Parametros de la estrategia (identicos a FT_SMC_v1) ----------------------
VENTANAS_VALIDAS    = (3, 5)
SCORE_ENTRADA_MIN   = 1
SCORE_MAXIMO        = 3
MIN_SL_DIST_PCT     = 1.0
MAX_SL_DIST_PCT     = 8.0
...
def nombre_estrategia(ventana: int) -> str:
    return f"FT_SMC_v3_N{int(ventana)}"
...
def evaluar_cierres(
    ...
    """
    Prioridades de salida (iguales a FT_SMC_v1):
        P0. Earnings manana     (prioridad absoluta, antes que todo)
        P1. Trailing SL roto    (precio <= stop_loss)
        P2. CHoCH bajista hoy   (choch_bear confirmado)
        P3. Estructura rota     (estructura = -1)
    ...
    datos_map = obtener_estructura_tickers(tickers, ventana)
    a_cerrar  = []
...
def procesar_trailing_sl(
    ...
    """
    Recalcula el SL estructural de cada posicion abierta sobre el ultimo swing low
    CONFIRMADO. Actualiza en DB solo si el nuevo SL es mayor al actual.
    """
    if not posiciones:
        return posiciones

    updates = calcular_actualizaciones_sl(posiciones, ventana)
    if not updates:
...
def evaluar_entradas(
    ...
    """
    Escanea la estructura confirmada y retorna candidatos para abrir posicion.
    """
    ...
    if features is None:
        features = obtener_features_hoy(ventana)
    candidatos = []
...
def run(ventana: int, dry_run: bool = False):
    hoy = date.today()
    sep = "-" * 55
    nombre = nombre_estrategia(ventana)

    log(sep)
    log(f"FT Bot SMC estructura confirmada N={ventana} | {hoy} "
        f"{'[DRY RUN]' if dry_run else ''}")
    log(sep)

    # 1. Cargar estrategia
    estrategia = cargar_estrategia(nombre)
    if not estrategia:
        log(f"[ERROR] Estrategia '{nombre}' no encontrada o inactiva.")
        return 1
    ...
    log(sep)
    log("TRAILING SL:")
    posiciones = procesar_trailing_sl(posiciones, precios, ventana, dry_run)

    # 6. Evaluar cierres
    log(sep)
    log("CIERRES:")
    a_cerrar = evaluar_cierres(posiciones, precios, earnings_cierre, ventana)
    ...
    features_hoy       = obtener_features_hoy(ventana)
...
def main():
    parser = argparse.ArgumentParser(
        description="FT Bot SMC sobre estructura confirmada (features_estructura)")
    parser.add_argument("--ventana", type=int, required=True, choices=VENTANAS_VALIDAS,
                        help="N de confirmacion del swing (3 o 5)")
    parser.add_argument("--dry-run", action="store_true",
                        help="Evalua sin escribir en DB")
    args = parser.parse_args()
    return run(ventana=args.ventana, dry_run=args.dry_run) or 0
