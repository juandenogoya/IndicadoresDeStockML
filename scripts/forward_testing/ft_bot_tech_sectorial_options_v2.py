...
Forward-testing — AT Tecnico Sectorial con confirmacion de opciones (PCR_VOL).

Estrategia FT_TECH_SECTOR_OPTIONS_v2:
    Identica a FT_TECH_SECTOR_OPTIONS_v1 en estructura, gates y parametros.
    UNICA diferencia: el Put/Call Ratio se calcula sobre el VOLUMEN diario
    de contratos (PCR_VOL) en lugar del open interest acumulado (PCR_OI).

    El volumen es mas reactivo que el OI: refleja la actividad del dia, no el
    posicionamiento acumulado. La tesis es que el PCR_VOL puede anticipar
    giros que el PCR_OI ve tarde.

    ENTRADA — dos gates en serie:
        Gate 1: tech_score >= 4.0 (identico a v1)
        Gate 2: pcr_score >= 2/3 ventanas con PCR_VOL < 1.0 (alcistas)
    ...
    VENTANAS de vencimiento (dias al vencimiento desde fecha_snapshot):
        Corto:  1 – 14 dias   (weeklies, mayor volumen)
        Medio:  15 – 45 dias  (mensuales)
        Largo:  46 – 90 dias  (medio plazo)

    PCR_VOL por ventana = sum(put_VOL) / sum(call_VOL) en esa ventana
        < 1.0  -> Alcista (A)
        >= 1.0 -> Bajista  (B)

    pcr_score = cantidad de ventanas Alcistas (0-3)
    Minimo de liquidez por ventana: sum(call_VOL + put_VOL) >= 500
...
Uso:
    python scripts/forward_testing/ft_bot_tech_sectorial_options_v2.py
    python scripts/forward_testing/ft_bot_tech_sectorial_options_v2.py --dry-run
"""
...
NOMBRE_ESTRATEGIA = "FT_TECH_SECTOR_OPTIONS_v2"
...
PCR_VOL_UMBRAL_ALCISTA = 1.0   # < 1.0 = Alcista, >= 1.0 = Bajista
...
def obtener_pcr_vol_por_ventanas(tickers: list[str]) -> dict[str, dict]:
    """
    Calcula el PCR_VOL para cada ventana de vencimiento de cada ticker.
    Usa el ultimo fecha_snapshot disponible en opciones_snapshot.

    Para cada ventana (corto/medio/largo):
        PCR_VOL = sum(put_VOL) / sum(call_VOL)  para los contratos en ese rango

    Veredicto por ventana:
        PCR_VOL < 1.0   -> 'A' (Alcista)
        PCR_VOL >= 1.0  -> 'B' (Bajista)
        VOL < MIN_VOL   -> None (sin liquidez suficiente)
    ...
    with engine.connect() as conn:
        ...
        # Volumen por ventana para todos los tickers en una sola query
    ...
    # Acumular volumen por ticker y ventana
    acum: dict[str, dict] = {}
    for r in rows:
        t    = r.ticker
    ...
    for t in tickers:
        ...
        def _veredicto(call_vol, put_vol):
            ...
            v   = "A" if pcr < PCR_VOL_UMBRAL_ALCISTA else "B"
            return v, pcr
...
def run(dry_run: bool = False):
    ...
    log(sep)
    log(f"FT Bot Tech Sectorial Options v2 | {hoy} "
        f"{'[DRY RUN]' if dry_run else ''}")
    log(f"  {N_SECTORES} sectores x ${SECTOR_BUDGET:,.2f} | "
        f"Position size: ${POSITION_SIZE:,.2f} | Max {MAX_POS_SECTOR} pos/sector")
    log(f"  Gate 2: pcr_score >= {PCR_SCORE_ENTRADA_MIN}/3 (PCR_VOL) | "
        f"Min VOL/ventana: {MIN_VOL_POR_VENTANA}")
    log(sep)
    ...
    # 3. PCR_VOL por ventana para TODOS los tickers del universo
    # (necesario tanto para cierres como para entradas)
    log(sep)
    log("CALCULANDO PCR_VOL POR VENTANAS...")
    todos_tickers = list({i["ticker"] for i in indicadores})
    pcr_map = obtener_pcr_vol_por_ventanas(todos_tickers)
            ...
            if not dry_run:
                detalle = {
                    **e["detalle"],
                    ...
                    "pcr_vol_corto":   pcr_d.get("pcr_corto"),
                    "pcr_vol_medio":   pcr_d.get("pcr_medio"),
                    "pcr_vol_largo":   pcr_d.get("pcr_largo"),
...
def main():
    parser = argparse.ArgumentParser(
        description="FT Bot Tech Sectorial + Confirmacion Opciones PCR_VOL"
    )
