"""
bot_tech_sector.py -- Bot 2 Alpaca (Plan B, Tarea 16): TECH_SECTOR_v1.

Lee la masticada senales_bot_diaria, decide con el cerebro COMPARTIDO
src/strategies/sectorial.py (usar_opciones=False, homologable a
FT_TECH_SECTOR_v1) y ejecuta via ejecucion_bot (cuenta "_2" / tablas
posiciones_bot_tech / operaciones_bot_tech).

Uso:
    python scripts/alpaca/bot_tech_sector.py --dry-run
    python scripts/alpaca/bot_tech_sector.py
"""
...
SECTOR_BUDGET  = round(CAPITAL_TOTAL / len(SECTORES_ACTIVOS), 2)   # 11.111,11
MAX_POS_SECTOR = 5
POSITION_SIZE  = round(SECTOR_BUDGET / MAX_POS_SECTOR, 2)          # 2.222,22

CFG = ConfigSectorial(
    nombre="TECH_SECTOR_v1",
...
BCFG = eb.BotAlpacaConfig(
    nombre="TECH_SECTOR_v1",
    account_suffix="_2",
    tabla_pos="posiciones_bot_tech",
    tabla_oper="operaciones_bot_tech",
)
...
def run(dry_run: bool = False, ignore_frescura: bool = False):
    sep = "-" * 60
    log(sep)
    log(f"BOT ALPACA 2 -- TECH_SECTOR_v1 {'[DRY RUN]' if dry_run else '[REAL]'}")
    log(f"  {len(SECTORES_ACTIVOS)} sectores x ${SECTOR_BUDGET:,.2f} | "
        f"pos ${POSITION_SIZE:,.2f} | max {MAX_POS_SECTOR}/sector")
    log(sep)
    ...
    a_abrir, _ = evaluar_entradas_sectorial(
        posiciones, indicadores, precios, bloqueados, tickers_cerrados, CFG, log=log,
    )
    if not a_abrir:
        log("  Sin entradas.")
    else:
...
def main():
    ap = argparse.ArgumentParser(description="Bot Alpaca 2 -- TECH_SECTOR_v1")
