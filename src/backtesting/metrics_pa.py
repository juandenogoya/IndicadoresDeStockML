"""
metrics_pa.py
Calculo de metricas de performance para el backtesting PA challenger.

Lee de operaciones_bt_pa, escribe en resultados_bt_pa.
Reutiliza calcular_metricas() de metrics.py (misma logica de calculo).
"""
...
# ─────────────────────────────────────────────────────────────
# Calculo y persistencia del resumen PA
# ─────────────────────────────────────────────────────────────

def calcular_y_guardar_resultados_pa(df_ops: pd.DataFrame):
    """
    Agrega operaciones PA por combinacion EV x SV x Ticker x Segmento,
    calcula metricas y persiste en resultados_bt_pa.

    Genera filas por ticker Y filas globales (ticker = NULL) para
    comparacion directa con el backtesting original.
    """
    if df_ops.empty:
        print("  [WARN] Sin operaciones PA para calcular metricas.")
        return
    ...
    print(f"  Resultados PA persistidos: {len(registros):,} filas en resultados_bt_pa.")
